...
# This will instal the ballgown packages.
```{r}
...
# This code is creating a data frame called "pheno_data". It is getting its data from the "fastq" folder filled with the different "fastq.qz" files.
```{r}
...
# <what is this code doing?>
# This code is giving us a new file named "big_filt" 
```{r}
...
```
```{r}
results_transcripts[results_transcripts$transcriptNames == "gene-PA0008", ]
```


## what information are you given about this transcript?
# We are given the quality %, the p-value, the fc number or significant differences, and how many characters there are in the transcript
# This code is going to create an object called "sigdiff" and it will be a filtered version of the results that are only p-values less than 0.05
```{r}
sigdiff <- results_transcripts %>% filter(pval<0.05)
dim(sigdiff)
```

# organize the table this table is going to be organized by decreasing "fc" value or the significant differences value
```{r}
...
# This code changed the names of the columns 
```{r}
...
## This graph is showing us that there is a higher frequency when there is less transcripts per gene. There are more genes with one transcript than genes with multiple transcripts. And like the graph says, the max amount of transcripts per gene is 64. 
...
# If you want to use different data sets, you would change the X and Y variables. For example you can use the biofilm01 and 02 instead of the plank01 and 02
```{r}
...
# I think that this means the data sets bot have the same distribution
...
## interpret the above figure: I think that this graph is showing us that most of the data falls either from -50 to 50. I am assuming from 0 to -50 is Planktonic or Biofilm, then from 0 to 50 is the other one.
...
# This figure is showing us that the two data sets are not the same. This means that the biofilm and planktonic data are significantly different. I think they are sig diff because the two box plots are not overlapping each other. 